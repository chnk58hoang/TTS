...
class Vits2DiscriminatorLoss(nn.Module):
    def __init__(self, c: Coqpit):
        super().__init__()
        self.disc_loss_alpha = c.disc_loss_alpha
...
class Vits2GeneratorLoss(nn.Module):
    def __init__(self, c: Coqpit):
        super().__init__()
        self.kl_loss_alpha_dur = c.kl_loss_alpha_dur
        self.kl_loss_alpha_audio = c.kl_loss_alpha_audio
        self.gen_loss_alpha = c.gen_loss_alpha
        self.gen_e2e_loss_alpha = c.gen_e2e_loss_alpha
        self.feat_loss_alpha = c.feat_loss_alpha
        self.dur_loss_alpha = c.dur_loss_alpha
        self.mel_loss_alpha = c.mel_loss_alpha
        self.spk_encoder_loss_alpha = c.speaker_encoder_loss_alpha
...
sdtw = SoftDTW(use_cuda=False, gamma=0.1, warp=134.4)
...
class NaturalSpeechGeneratorLoss(nn.Module):
    def __init__(self, c: Coqpit):
        super().__init__()
        self.kl_loss_alpha = c.kl_loss_alpha
        self.kl_loss_fwd_alpha = c.kl_loss_fwd_alpha
        self.gen_loss_alpha = c.gen_loss_alpha
        self.gen_e2e_loss_alpha = c.gen_e2e_loss_alpha
        self.feat_loss_alpha = c.feat_loss_alpha
        self.dur_loss_alpha = c.dur_loss_alpha
    ...
    def forward(
        ...
        Shapes:
            ...
            - m_p: :math:`[B, C, T]`
            - logs_p: :math:`[B, C, T]`
            - z_len: :math:`[B]`scores_disc_fake_e2e, _, scores_disc_real_e2e, _ = self.disc(
        ...
        if not use_sdtw:
            ...
            loss_kl_fwd = self.kl_loss(z_p=z_q, logs_q=logs_p, m_p=m_q, logs_p=logs_q,
                                       z_mask=p_mask.unsqueeze(1)) * self.kl_loss_fwd_alpha
        else:
            loss_kl = self.kl_loss_dtw(z_p=z_p, logs_q=logs_q,
                                       m_p=m_p, logs_p=logs_p,
                                       p_mask=p_mask, q_mask=z_mask) * self.kl_loss_alpha
            loss_kl_fwd = self.kl_loss_dtw(z_p=z_q, logs_q=logs_p,
                                           m_p=m_q, logs_p=logs_q,
                                           p_mask=z_mask, q_mask=p_mask) * self.kl_loss_fwd_alpha
        # feature match loss
        loss_feat = (
            self.feature_loss(feats_real=feats_disc_real, feats_generated=feats_disc_fake) * self.feat_loss_alpha
        )
        ...
        # duration loss
        loss_duration = torch.sum(loss_duration.float()) * self.dur_loss_alpha
        loss = loss_kl + loss_feat + loss_mel + loss_gen + loss_duration + loss_gen_e2e + loss_kl_fwd
        if use_speaker_encoder_as_loss:
            loss_se = self.cosine_similarity_loss(gt_spk_emb, syn_spk_emb) * self.spk_encoder_loss_alpha
            loss = loss + loss_se
            return_dict["loss_spk_encoder"] = loss_se
        ...
        return_dict["loss_kl"] = loss_kl
        return_dict["loss_kl_fwd"] = loss_kl_fwd
        return_dict["loss_feat"] = loss_feat
        return_dict["loss_mel"] = loss_mel
        return_dict["loss_duration"] = loss_duration
        return_dict["loss"] = loss
...
class OVDiscriminatorLoss(nn.Module):
    def __init__(self, c: Coqpit):
        super().__init__()
        self.disc_loss_alpha = c.disc_loss_alpha
...
class OVGeneratorLoss(nn.Module):
    def __init__(self, c: Coqpit):
        super().__init__()
        self.kl_loss_alpha = c.kl_loss_alpha
        self.gen_loss_alpha = c.gen_loss_alpha
    ...
    def forward(self,
        ...
        z_mask = sequence_mask(z_len).float()
        # compute losses
        loss_kl = self.kl_loss(z_p, logs_q, m_p_aligned, logs_p_aligned, z_mask.unsqueeze(1)) * self.kl_loss_alpha
        loss_feat = self.feature_loss(feats_real=feats_disc_real, feats_generated=feats_disc_fake) * self.feat_loss_alpha
        loss_gen = self.generator_loss(scores_fake=scores_disc_fake)[0] * self.gen_loss_alpha
        loss_mel = torch.nn.functional.l1_loss(mel_slice, mel_slice_hat) * self.mel_loss_alpha
